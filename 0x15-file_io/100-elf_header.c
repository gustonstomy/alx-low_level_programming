...
#include <unistd.h>
/**
 * _sting_compair - comparison of  two strings
 ...
 */
int  _sting_compair(const char *s1, const char *s2, size_t n)
{
	for ( ; n && *s1 && *s2; --n, ++s1, ++s2)
	{
		if (*s1 != *s2)
 ...
 */
void print_magic(const unsigned char *buffer)
{
	unsigned int i;

	if (_sting_compair((const char *) buffer, ELFMAG, 4))
	{
 ...
 */
size_t print_class(const unsigned char *buffer)
{
	printf("  %-34s ", "Class:");
 ...
 */
int print_data(const unsigned char *buffer)
{
	printf("  %-34s ", "Data:");
 ...
 */
void print_version(const unsigned char *buffer)
{
 ...
 * @buffer: the header of ELF
 */
void print_osabi(const unsigned char *buffer)
{
...
/**
 * print_abivers - it prints ELF ABI version
 * @buffer: the header of ELF
 */
void print_abivers(const unsigned char *buffer)
{
	printf("  %-34s %u\n", "ABI Version:", buffer[EI_ABIVERSION]);
}

/**
 * print_type - it prints ELF type
 * @buffer: the  header of ELF
 * @big_endian: big endian if non-zero
 */
void print_type(const unsigned char *buffer, int big_endian)
{
	char *type_table[5] = {
		"NONE (No file type)",
...
/**
 * print_entry - to print entry point
 * @buffer: string containing the entry point address
 * @bit_mode: 32 or 64
 * @big_endian: big endian if non-zero
 */
void print_entry(const unsigned char *buffer, size_t bit_mode, int big_endian)
{
...
int main(int argc, const char *argv[])
{
	...
	big_endian = print_data(buffer);
	print_version(buffer);
	print_osabi(buffer);
	print_abivers(buffer);
